...
const attachGetter = (obj, propName, getters) => {
  Object.defineProperty(propName, propName, {
    get: () => {
...
export default function generateSynapseRecord(defaultState: any, stateKey: String, getters: Object) {
  const tempValues = defaultState.toJS ? defaultState.toJS() : defaultState;
  ...
  class SynapseRecord extends Record(tempValues) {
    constructor(defaults) {
      ...
      if (getters) {
        ...
        for (let i = 0; i < keys.length; i++) {
          attachGetter(this.prototype, keys[i], getters);
        }
      }
     ...
     */
    reattachSynapseContext(record, newStateKey = this.__stateKey__) {
      record.__stateKey__ = newStateKey;
      return record;
    }
